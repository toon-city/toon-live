...
mirrors as `80 - x - w`, no separate source art needed). The raw Flash
export was done at 250% instead of the intended 150% — every source frame
needed a `150/250 = 0.6` rescale before anything else, or the garment comes
out visibly oversized relative to the body (caught by comparing a raw
frame's pixel width, e.g. 58px for direction 1, against the body's own
same-direction leg bbox, ~39px — a ratio close to 250/150, not something a
shorts-vs-legs fit difference would explain on its own). Per-direction
canvas placement (after rescaling) was found by compositing each
direction's first frame against the real body reference
(`game-core/assets/toon/sprites/human_lg_{d}_0.png` / `human_bd_{d}_0.png`)
at increasing opacity until the waistband met the torso's bottom edge with
no gap — the build script is `/tmp/.../scratchpad/pant_work/build_pant2.py`
from the session that produced it, not checked into this repo (one-off per
source art dump, not a general tool — that's what clothing-studio is for
going forward). **If a future raw export needs the same 250→150 correction,
that's a real red flag the export step itself should be fixed at the
source, not something to re-derive by eye each time.**
